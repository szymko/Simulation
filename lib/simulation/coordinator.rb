...
module Simulation
  ...
  class Coordinator
    ...
    def_delegator :@system, :time, :time
    ...
    # stops simulation, kills threads associated with each process
    def stop
      @current_list.kill
    end
    ...
    def move_to_current
      current = @future_list.reactivate_now
      current_time = current.empty? ? time : current.first.reactivation_time
